...
def plot_stacked_bars(is_edge):
    ...
    all_results = np.full((num_simulations, len(device_counts), 2), np.nan)

    for s in range(1, num_simulations + 1):
        for j, num_devices in enumerate(device_counts):
            try:
                # Note: This script uses a specific scenario name from the original .m file
                file_name = f'SIMRESULT_SCENARIO1_EDGE_ONLY_{num_devices}DEVICES_ALL_APPS_GENERIC.log'
                file_path = os.path.join(folder_path, f'ite{s}', file_name)
                data = pd.read_csv(file_path, sep=';', header=None, skiprows=1, nrows=5)
                value1 = data.iloc[1, 5] if is_edge else data.iloc[2, 5] # row 2/3, col 6
                value2 = data.iloc[4, 0] if is_edge else data.iloc[4, 2] # row 5, col 1/3
                all_results[s, j, :] = [value1, value2]
            except Exception:
                continue
    ...
    bar_width = step_devices * 0.8 # Make bars proportional to step size
    ...
    ax.set_xlim(start_devices - step_devices, end_devices + step_devices)
    fig.tight_layout()
...
if __name__ == '__main__':
    ...
    plot_stacked_bars(is_edge=True)
    plot_stacked_bars(is_edge=False)